...
function addEvenToCarousel(carouselElem) {
	...
	var liWidth = liElems[0].clientWidth; //600을 담아둠
	var adjustedWidth = liElems.length * liWidth; //2400을 담아둠
	ulElem.style.width = adjustedWidth;

	//슬라이드 버튼 이벤트 
	var slideButtons = carouselElem.querySelectorAll(".slide");
	//slide-left slide-right 두 개이므로 for문 사용하여 클릭 이벤트(createListenerSlide()호출)
...
function createListenerSlide(carouselElem) {
	return function(event) {
		...
		//liCount - 1: 5개일 때 0, 1, 2, 3, 4 이므로 인덱스번호를 -1 처리 0,1,2,3
...
function scrollDiv(carouselElem, nextIndex) {
	...
	var liWidth = scrollable.clientWidth; //div 안의 가로 넓이
...
//window.addEventListener("load", function()): 페이지가 완전히 로드된 후에 함수를 실행하는 방법
